# -*- coding: utf-8 -*-
"""
Created on Wed Oct 23 12:46:53 2024
...
#df = df.dropna(subset=['baseline -0'])
df = df.drop(columns=[f"baseline -{k}" for k in range(0,5)])
#df = df[df['speaker'].isin(range(1, 5))]
df = df[df['speaker']==1]

warnings.filterwarnings("ignore")

def add_column(df, emotion):
    ...
    for fold in df['fold'].unique():
        ...
        test_data.loc[:, f"baseline {emotion} model"] = y_pred
...
for emotion in [0,1,2,3,4]:
    ...
    results_df = add_column(emotion_data, emotion)
    df = pd.merge(df, results_df, how='left')

columns_to_merge = [f"baseline {emotion} model" for emotion in range(0,5)]
df["baseline model"] = df[columns_to_merge].bfill(axis=1).iloc[:, 0]
df = df.drop(columns=columns_to_merge)

results_path = os.path.join('..','podaci','text-to-speech', 'liste', "baseline_results.csv")
