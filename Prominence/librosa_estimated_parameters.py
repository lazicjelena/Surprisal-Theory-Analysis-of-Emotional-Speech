# -*- coding: utf-8 -*-
"""librosa_estimated_parameters.py
...
Pipeline role
-------------
Per-word prosodic feature extractor for the alternative,
librosa-based prominence pipeline. Walks
``../podaci/data_mono/<speaker>/<emotion>/*.wav`` and the parallel
``../podaci/transcript_corrected/`` tree, and for every word slice
in each utterance computes the mean fundamental frequency
(:func:`librosa.pyin` with gender-specific ``fmin``/``fmax``) and
mean RMS energy (:func:`librosa.feature.rms`). The canonical
target-sentence id is recovered by case-insensitive lookup in
``../podaci/target_sentences.csv``; words that the wavelet GUI
glued together are split via :func:`text_utils.find_subword`.
The local helper :func:`lookup_features` then joins per-word
GPT-2/Yugo/BERT/BERTic/ngram-3 surprisals from the corresponding
``../podaci/word_surprisals_*.csv`` tables, and the result is
written to ``../podaci/prosody_surprisal.csv`` as the input for
downstream correlation reporting.
"""
