...
Pipeline role
-------------
Post-wavelet move/cleanup step in the ``Prominence/`` chain.
Moves every ``*.prom`` file from the wavelet GUI's flat
``../../wavelet_prosody_toolkit/data_prosody/all_files`` working
directory into the project's ``../podaci/prosody`` data folder,
then deletes the matching ``*.wav`` and ``*.lab`` source files
