...
Pipeline role
-------------
Surprisal-functional-form sweep. Loads the master training
table ``../podaci/training_data.csv`` and, for every surprisal
column in the configured list, sweeps the wrapping function
(``power``, ``linear``, ``logarithmic``, ``exponential``)
through :func:`my_functions.inf_k_model`, then per
``(emotion, k, function)`` reports the delta log-likelihood
improvement over the surprisal-free baseline via
:func:`my_functions.calculate_delta_ll`. Output figures show
how the choice of functional form affects the predictive power
of each surprisal model.
"""
