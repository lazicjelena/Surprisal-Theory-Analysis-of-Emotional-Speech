...
Pipeline role
-------------
Multi-model duration regression sweep. Loads the master training
table ``../podaci/training_data.csv`` (restricted to the
selected surprisal columns from BERT, BERTic, RoBERTa, ALBERT
and their unidirectional variants), iterates over the
surprisal exponent ``k`` via :func:`my_functions.inf_k_model`
and per ``(emotion, gender, k)`` reports the delta
log-likelihood improvement over the surprisal-free baseline
via :func:`my_functions.calculate_delta_ll`. The resulting
plots compare the predictive value of bidirectional vs
unidirectional contextual surprisals across the additional
post-recension models.
"""
