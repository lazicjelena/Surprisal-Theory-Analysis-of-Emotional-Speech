...
Pipeline role
-------------
Per-word and per-position breakdown of the surprisal-augmented
duration regression for the post-recension dataset. Loads
``../podaci/training_data.csv`` (joined against
``target_sentences.csv``), sweeps the surprisal exponent ``k``
via :func:`my_functions.inf_k_model` and per word / per
sentence position reports the delta log-likelihood improvement
over the surprisal-free baseline via
:func:`my_functions.akaike_for_column`, quantifying which
words and which positions in the sentence gain the most
