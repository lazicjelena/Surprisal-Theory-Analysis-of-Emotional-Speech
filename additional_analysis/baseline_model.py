...
Pipeline role
-------------
Surprisal-free baseline trainer for the post-recension
duration analysis. Loads ``../podaci/training_data.csv`` and
trains a fold-wise :class:`sklearn.linear_model.LinearRegression`
on ``log2(time)`` with predictors ``[length, log probability]``
and a 3-sigma outlier filter on the training fold. The
out-of-fold predictions are written into a ``baseline``
column and saved back to ``training_data.csv``; this column is
