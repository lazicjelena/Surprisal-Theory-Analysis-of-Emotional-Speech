...
Pipeline role
-------------
Master training-table builder for the post-recension duration
analysis. Recursively walks ``../podaci/data/<speaker>/<emotion>/``
forced-alignment CSVs and concatenates them, joins per-word log
probabilities from ``../podaci/wordlist_frequencies.csv`` via
:func:`my_functions.lookup_features` and per-word grammatical
type tags from ``../podaci/word_type.csv`` via
:func:`my_functions.add_word_type`, and joins fold ids from
``../podaci/folds.csv``. The resulting ``training_data.csv`` is
the canonical input for ``baseline_model.py``,
``additional_models_results.py``,
``additional_functions_results.py``,
``analysis_accross_individual_speakers.py``,
``analysis_accross_individual_words.py``, and
