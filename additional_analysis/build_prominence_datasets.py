...
Pipeline role
-------------
Master prosody-table builder for the post-recension prosody
analyses. Walks the wavelet-prosody-toolkit ``.prom`` outputs
in ``../podaci/prosody 1 0 0/``, parses each line into
``(speaker, emotion, word, prominence, boundary, duration)``
records and reconstructs each utterance's target-sentence id
through :func:`my_functions.most_similar_sentence_index`. Words
that fail the canonical-vocabulary check are split into the
longest-suffix subwords via :func:`find_subword`. Per-word
surprisals from BERT, BERTic, ngram-3, GPT-2 and Yugo are
joined via :func:`my_functions.lookup_features`, and the table
is finally augmented with speaker gender (``gender_data.csv``)
and fold ids (``folds.csv``) before being written to
