...
Pipeline role
-------------
Final plotting script for the prosody-prediction sweep. For a
chosen prosody target (e.g. energy / f0) loads the per-prosody
table ``../podaci/<prosody>_data.csv`` (built by
``build_prominence_datasets.py`` plus
``baseline_model_prosody.py``), sweeps the surprisal exponent
``k`` over every model in the surprisal list via
:func:`my_functions.inf_k_model` and per
``(emotion, gender, k)`` plots the delta log-likelihood
improvement over the surprisal-free baseline via
:func:`my_functions.calculate_delta_ll`. The output is purely
...
for gender in ['f', 'm']:
    ...
    for emotion in [0,1,2,3,4]:
        ...
        for surprisal in surprisal_list:
            ...
            for i in x_axis:
                k = round(i, 2)
                delta_element,  std_element = calculate_delta_ll(mode="prominence", data=emotion_data, surprisal_name=surprisal, k=k, prominence=prosody)
