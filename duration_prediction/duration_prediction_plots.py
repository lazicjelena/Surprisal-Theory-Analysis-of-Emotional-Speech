...
Pipeline role
-------------
Final plotting script for the duration-regression sweep over a
continuous surprisal exponent ``k``. Reads the per-surprisal
``\Delta\log\mathcal{L}`` summary tables produced by
``surprisal_results.py``
(``../podaci/results - linear regression/<Surprisal X>_results.csv``,
one row per ``(speaker gender, emotion, k)`` with
``k`` in ``np.arange(0.25, 3, 0.25)``) and renders the
publication figures: per-emotion / per-gender scatters of
``\Delta\log\mathcal{L}`` versus the continuous surprisal
power ``k``, in two color groups (autoregressive: GPT-2 vs.
Yugo vs. ngram-3; bidirectional: BERT vs. BERTic), each in both
English and Serbian variants. Output is purely visual; no CSVs
are written.
"""
...
file_path = os.path.join('..','podaci', 'results - linear regression')
