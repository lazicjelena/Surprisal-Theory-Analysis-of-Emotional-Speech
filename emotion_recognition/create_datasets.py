...
Pipeline role
-------------
Train / val / test split builder for the emotion-recognition
model. Walks ``data_mono/<speaker>/<emotion>/*.wav``, takes the
inner folder name as the integer emotion label, and uses
:func:`sklearn.model_selection.train_test_split` (with
stratification on the label) to produce a 65 / 20 / 15 split
written to ``data/train_dataset.csv``,
``data/val_dataset.csv`` and ``data/test_dataset.csv``. The
split CSVs feed both ``training.py`` /
``emotion_recognition_model.py`` (via ``audiodataset.py``).
"""
