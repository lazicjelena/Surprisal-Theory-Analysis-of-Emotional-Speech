...
Pipeline role
-------------
CPU training script for the ``MyModel`` emotion-recognition
classifier (Colab-targeted variant of ``training.py``). Reads
``data/train_dataset.csv`` and ``data/val_dataset.csv`` (built by
``create_datasets.py``), wraps each in a shuffling DataLoader via
:func:`audiodataset.create_dataloader`, instantiates ``MyModel``
with ``num_classes = 5`` and trains it under cross-entropy with
Adam (``lr = 0.0006``) for 30 epochs. Per-epoch (train_loss,
val_loss, num_class_train, num_class_val) is appended to
``training_log.csv`` and a checkpoint is written after every
epoch to a Google Drive path
