# -*- coding: utf-8 -*-
"""Learning Rate Emotion Recognition.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/17pXsC7_siuuLwHP3LYbbKN4WGpBQEZfn

Pipeline role
-------------
Training-curve plotting script for the emotion-recognition model.
Reads ``model/training_log.csv`` (appended to per epoch by
``emotion_recognition_model.py`` / ``training.py``) and renders a
training-vs.-validation loss curve in two language variants -
English and Cyrillic Serbian - both with a grey shaded region up
to epoch 2 marking the early-warmup phase. Output is purely
visual; no CSVs are written.
"""

import matplotlib.pyplot as plt
import pandas as pd

df = pd.read_csv('model/training_log.csv')

# Assuming df is your dataframe and 'Epoch' is already the index
plt.figure(figsize=(14, 8))
plt.plot(df.index, df['Train Loss'], label='Train Loss', marker='o', markersize=15, linestyle='-')
plt.plot(df.index, df['Val Loss'], label='Validation Loss', marker='o', markersize=15, linestyle='-')
#plt.xticks(df.index)
plt.xlabel('Epoch', fontsize=25)
plt.ylabel('Loss', fontsize=25)
plt.title('Training and Validation Loss Over Epochs', fontsize=25)

#plt.axvline(x=7, color='k', linestyle='-', label='Learning rate change')
# Add shaded region before the vertical line
plt.axvspan(xmin=min(plt.xlim()[0], 2), xmax=2, color='grey', alpha=0.3)
# Add shaded region after the vertical line
plt.axvspan(xmin=2, xmax=max(plt.xlim()[1], 2), color='lightgrey', alpha=0.3)

plt.xticks(fontsize=20)
plt.yticks(fontsize=20)
plt.legend(fontsize=20)
plt.show()


""" Grafici na srpskom """

plt.figure(figsize=(14, 8))
plt.plot(df.index, df['Train Loss'], label='Тренинг скуп података', marker='o', markersize=15, linestyle='-')
plt.plot(df.index, df['Val Loss'], label='Валидациони скуп података', marker='o', markersize=15, linestyle='-')
#plt.xticks(df.index)
plt.xlabel('Епоха', fontsize=25)
plt.ylabel('Функција губитка', fontsize=25)
plt.title('Функција губитка на тренинг и валидационом скупу података', fontsize=25)

#plt.axvline(x=7, color='k', linestyle='-', label='Learning rate change')
# Add shaded region before the vertical line
plt.axvspan(xmin=min(plt.xlim()[0], 2), xmax=2, color='grey', alpha=0.3)
# Add shaded region after the vertical line
plt.axvspan(xmin=2, xmax=max(plt.xlim()[1], 2), color='lightgrey', alpha=0.3)

plt.xticks(fontsize=20)
plt.yticks(fontsize=20)
plt.legend(fontsize=20)
plt.show()