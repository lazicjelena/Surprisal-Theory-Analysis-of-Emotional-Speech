...
Pipeline role
-------------
Test-set evaluation script for the emotion-recognition model.
Loads a per-row ``(file_path, label)`` CSV (the active source is
``../Emotion Text to Speech 2/data.csv``; alternative
``data/google_speech_data.csv``,
``data/baseline_data.csv`` and ``data/surprisal_data.csv``
sources are listed but commented out), wraps it in
:func:`audiodataset.create_dataloader`, restores ``MyModel`` from
``model/model_epoch_30.pth``, runs inference and prints the
overall accuracy ``trace(cm) / sum(cm) * 100``. A Cyrillic
Serbian confusion matrix is then rendered via
:class:`sklearn.metrics.ConfusionMatrixDisplay`; the equivalent
