...
Pipeline role
-------------
GPU-aware training driver for the ``MyModel``
emotion-recognition classifier. Identical loop to
``emotion_recognition_model.py`` but moves both the model and
each batch to ``cuda`` when available. Reads
``data/train_dataset.csv`` and ``data/val_dataset.csv`` (built by
``create_datasets.py``), trains under cross-entropy with Adam
(``lr = 0.0006``) for 30 epochs, appends the per-epoch
(train_loss, val_loss, num_class_train, num_class_val) to
``training_log.csv`` and writes a checkpoint after every epoch to
``model/model_epoch_<epoch>.pth``. Output checkpoints are then
