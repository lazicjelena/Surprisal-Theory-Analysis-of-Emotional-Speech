...
Pipeline role
-------------
End-to-end significance-test driver for adjusted surprisal vs.
information value as duration predictors. Loads the master table
``../podaci/information measurements parameters/data.csv``,
restricts to non-zero ``time``, and for every
``(parameter, emotion, gender)`` triple sweeps every information
measurement column against a fold-wise linear regression model
that adds the parameter on top of GPT-2 surprisal. Per
``(emotion, gender)`` the helpers from ``my_functions`` are used
to compute the delta log-likelihood improvement over baseline
and the paired permutation p-value, which are written to
``../podaci/information measurements parameters/<...>_results.csv``
and consumed by ``plot_results.py``.
"""
