...
Pipeline role
-------------
Per-speaker breakdown of the transformer-surprisal regression
analysis. For one chosen model (BERTic / BERT / GPT) the script
loads ``../podaci/transformer layers parameters/datasets/<model>.csv``,
trains a fold-wise regression with the surprisal column added
on top of the ``baseline -3`` predictions and, for every
speaker, computes the delta log-likelihood improvement over
the surprisal-free baseline. The resulting per-speaker table
quantifies how much the chosen surprisal contributes to spoken
