...
Pipeline role
-------------
Master training-table builder for the
``Different information measurement parameters/`` analyses.
Starts from the ``Surprisal GPT-2`` table produced by
``Duration Prediction based on Surprisals/build_surprisal_datasets.py``
and joins three additional information-measurement parameters:
contextual entropy
(``../podaci/information measurements parameters/contextual_entropy.csv``),
information value
(``../podaci/information measurements parameters/information_value.csv``)
and adjusted surprisal
(``../podaci/information measurements parameters/adjusted_surprisal.csv``).
Each parameter is mapped onto the master table via
:func:`my_function.lookup_features` keyed by
``(target sentence, word)``. The resulting ``data.csv`` is the
