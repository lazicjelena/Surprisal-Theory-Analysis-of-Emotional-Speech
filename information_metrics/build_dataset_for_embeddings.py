...
Pipeline role
-------------
Embedding-feature joiner for the embedding-based information
measurement analyses. Reads either the information value or the
adjusted surprisal embedding source CSV (selected via the
``inform_value`` flag) from
``../podaci/information measurements parameters/`` and, via
:func:`my_functions.lookup_features` keyed by
``(target sentence, word)``, joins both context and non-context
embedding parameters onto the master ``Surprisal GPT-2`` table.
The two output tables (``context_embedding_data*.csv`` and
``non_context_embedding_data*.csv``) feed the
``baseline_results.py`` and ``iv_embedding_results.py`` sweeps.
"""
import os
import pandas as pd
