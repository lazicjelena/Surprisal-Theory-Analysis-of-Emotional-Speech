...
Pipeline role
-------------
Residual distribution diagnostic for the transformer-surprisal
regression. For one chosen model (BERTic / BERT / GPT) loads
``../podaci/transformer layers parameters/datasets/<model>.csv``,
fits a fold-wise regression with the surprisal added on top of
the ``baseline -3`` predictions and plots the histogram of the
prediction residuals against the histogram of the baseline
residuals. The figure is purely diagnostic and supports the
``akaike_for_column``-based delta log-likelihood numbers used
