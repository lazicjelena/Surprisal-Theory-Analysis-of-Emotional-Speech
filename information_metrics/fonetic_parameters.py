...
Pipeline role
-------------
Phonetic-feature regression sweep. Joins the per-word phonetic
features from
``../podaci/information measurements parameters/fonetic_features1.csv``
onto the master ``Surprisal GPT-2`` training table, then trains
a fold-wise regression with :func:`my_functions.fonetic_model`
that combines ``[length, log probability, Surprisal GPT-2]``
plus their per-row lags with the phonetic feature columns. Per
``(emotion, gender)`` the paired permutation test
(:func:`my_functions.paired_permutation_test`) and the delta
log-likelihood improvement
(:func:`my_functions.calculate_delta_ll`) are reported,
quantifying how much the phonetic features contribute on top
