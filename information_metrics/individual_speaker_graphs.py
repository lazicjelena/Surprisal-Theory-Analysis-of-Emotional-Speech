...
Pipeline role
-------------
Per-speaker visualisation driver for the transformer-surprisal
regression. For one chosen model (RoBERTa / ALBERT / BERTic /
BERT / GPT) and one speaker gender loads
``../podaci/transformer layers parameters/datasets/<model>.csv``,
fits a fold-wise regression with the surprisal added on top of
the ``baseline -3`` predictions and plots, for every speaker,
the per-emotion delta log-likelihood improvement over the
