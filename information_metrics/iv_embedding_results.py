...
Pipeline role
-------------
Embedding-based information value / adjusted surprisal
significance-test driver. Selects a context vs non-context
table and a surprisal vs information value table from
``../podaci/information measurements parameters/`` (via the
``context_ind`` and ``surprisal_ind`` switches), trains a
fold-wise regression with the embedding parameter added on top
of GPT-2 surprisal, and per ``(emotion, gender)`` reports the
delta log-likelihood improvement
(:func:`my_functions.calculate_delta_ll`) and the paired
permutation p-value
(:func:`my_functions.paired_permutation_test`). The figures are
generated in both English and Serbian variants and plotted
