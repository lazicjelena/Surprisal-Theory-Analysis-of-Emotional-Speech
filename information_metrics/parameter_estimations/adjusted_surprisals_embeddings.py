# -*- coding: utf-8 -*-
"""adjusted_surprisals_embeddings.py

Per-word *adjusted surprisal* across all 12 GPT-2 hidden layers, under
both contextual and non-contextual embedding similarity. Converted
from ``Adjusted Surprisals Embeddings.ipynb`` (P-013-NB).

Pipeline role
-------------
Layer-resolved variant of :mod:`adjusted_surprisals`, restricted to
the embedding-based similarity hooks. For every sentence in
...
``j in 1..12`` accumulates two context-probability-weighted
similarities per slot (contextual embedding cosine, non-contextual
embedding cosine), then converts each accumulator to
``-log2(acc + 1e-40)``. The result is written incrementally to
``../../podaci/embedding_adjusted_surprisal1.csv``.
...
OUTPUT_CSV_PATH = os.path.join('..', '..', 'podaci', 'embedding_adjusted_surprisal1.csv')
...
def calculate_word_adjusted_surprisal(sentence, vocabulary_df, model = model, tokenizer = tokenizer):
    """Per-word, per-layer adjusted surprisal for one sentence.
    ...
    similarities against the original word (contextual embedding
    cosine, non-contextual embedding cosine). After each layer's
    candidate sweep, both accumulators are converted to
    ``-log2(acc + 1e-40)``.
    ...
    ce_iv : list of list of float
        Adjusted surprisal under contextual embedding similarity,
        per layer (index ``1..12``).
    nce_iv : list of list of float
        Adjusted surprisal under non-contextual embedding similarity.
    """
...
def main():
    """Build and save the per-word, per-layer adjusted-surprisal table.

    Reads :data:`TARGET_SENTENCES_PATH` and :data:`VOCABULARY_PATH`,
    iterates target sentences over the range ``35..50`` (preserved
    verbatim from the source notebook), computes per-word per-layer
    adjusted surprisals via :func:`calculate_word_adjusted_surprisal`,
    and writes the incremental ``(Sentence, Word, CE 1..12,
    NCE 1..12)`` table to :data:`OUTPUT_CSV_PATH` after each
    sentence.
    """
