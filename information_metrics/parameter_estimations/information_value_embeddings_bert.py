# -*- coding: utf-8 -*-
"""information_value_embeddings_bert.py

Per-word *information value* across all 12 BERT hidden layers, under
contextual embedding similarity. Converted from
``Information Value Embeddings BERT.ipynb`` (P-013-NB).

Pipeline role
-------------
BERT (multilingual masked-LM) variant of
...
candidates from
``../../podaci/wordlist_classlawiki_sr_cleaned.csv``, queries
``bert-base-multilingual-uncased`` with ``output_hidden_states=True``,
and for each hidden-layer index ``j in 1..12`` accumulates a
context-probability-weighted contextual-embedding distance per slot
(column ``CE j``). The result is written incrementally to
``../../podaci/information_value_3.csv``.

Notes
-----
- Requires the ``transformers`` and ``torch`` packages.
- Module-level model / tokenizer are loaded eagerly so the default
  arguments of the helpers resolve correctly.
"""
...
OUTPUT_CSV_PATH = os.path.join('..', '..', 'podaci', 'information_value_3.csv')
...
def main():
    ...
    iterates target sentences from index ``39`` onwards (preserved
    ...
    for i in range(39,len(target_sentences_df)):
