# -*- coding: utf-8 -*-
"""information_value_embeddings_roberta.py

Per-word *information value* across all 12 hidden layers under
contextual embedding similarity. Converted from
``Information Value Embeddings RoBERTa.ipynb`` (P-013-NB).

Pipeline role
-------------
"RoBERTa" / ALBERT variant of :mod:`information_value_embeddings`.
The source notebook contains two model-loading cells: a (commented
out) ``FacebookAI/roberta-base`` block and an active
``albert-base-v2`` block; the latter is the one that runs in the
notebook and is preserved here. To switch to RoBERTa, swap the
``model``/``tokenizer`` instantiation below for the commented
RoBERTa snippet from the source notebook.

For every sentence in ``../../podaci/target_sentences.csv``, iterates
over each word slot, substitutes a 50-row ``random_state=42`` sample
of vocabulary candidates from
...
``../../podaci/information_value_2.csv``.

The source notebook also concluded with two cells that concatenated
``information_value_0.csv``, ``information_value_1.csv`` and
``information_value_2.csv`` into ``information_value_albert.csv``;
that merge step is omitted here.
...
OUTPUT_CSV_PATH = os.path.join('..', '..', 'podaci', 'information_value_2.csv')

# Model configuration.
# Active model in the source notebook (a commented-out RoBERTa block
...
def main():
    ...
    iterates target sentences from index ``46`` onwards (preserved
