...
Pipeline role
-------------
Final plotting script for the surprisal vs contextual entropy
sweep. Reads the per-parameter delta log-likelihood summary
table from
``../podaci/information measurements parameters/results.csv``
(produced by ``surprisal_vs_entropy.py`` and friends) and
renders the publication figure: per-emotion / per-gender bars
of ``\Delta\log\mathcal{L}`` with paired-permutation
significance markers, in both English and Serbian variants.
The output is purely visual; no CSVs are written.
"""
