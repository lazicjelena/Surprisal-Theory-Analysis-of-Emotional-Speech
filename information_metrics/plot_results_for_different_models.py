# -*- coding: utf-8 -*-
"""plot_results_for_different_models.py
...
Pipeline role
-------------
Multi-model plotting driver. For one chosen surprisal model
(RoBERTa / ALBERT / BERTic / BERT / GPT) loads
``../podaci/transformer layers parameters/datasets/<model>.csv``,
fits a fold-wise regression with the surprisal added on top of
the ``baseline -3`` predictions and per ``(emotion, gender)``
plots the delta log-likelihood improvement
(:func:`my_functions.calculate_delta_ll`) and the paired
permutation p-value
(:func:`my_functions.paired_permutation_test`). The figures
...
for parameter in parameters:
    ...
    for emotion in [0,1,2,3,4]:
        ...
        delta_element, _ = calculate_delta_ll(mode="flexible", data=emotion_data, model_name=f"{surprisal} {parameter} model", baseline=baseline_model)
