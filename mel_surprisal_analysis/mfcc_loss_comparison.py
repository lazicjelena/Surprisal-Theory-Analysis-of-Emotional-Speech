...
Pipeline role
-------------
Final reporting script for the surprisal/mel pipeline. Reads
three parallel result directories under ``../podaci/mel results/``
(``Results random``, ``Results keras``, ``Results surprisal``),
each containing per-fold ``results_surprisal_<k>.csv`` produced
by ``model.py`` runs with different conditioning, concatenates
them across folds, renames each ``Test Loss`` column after its
source directory (``random loss``, ``keras loss``,
``surprisal loss``) and merges them on
``(Audio Files, Speaker, Emotion)``. Speaker gender is joined in
from ``../podaci/gender_data.csv`` and the table is restricted to
a single gender (``gender = 'm'`` by default). Per-emotion mean
and standard deviation are printed for each loss column and a
per-emotion side-by-side boxplot is rendered. Output is purely
textual + visual; no CSVs are written.
"""
