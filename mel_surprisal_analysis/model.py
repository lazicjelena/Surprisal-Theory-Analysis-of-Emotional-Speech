...
Pipeline role
-------------
Main training script for the Tacotron-style spectrogram predictor
in ``Mel coefficients and surprisals/``. Reads
``../podaci/general_data.pkl`` (text/surprisal/mel triples
produced by ``calculate_mel_spectrum.py``), remaps the original
10-fold split to a 5-fold split (``Fold`` column), holds out fold
``k_test = 4`` for testing and ``k_val = 0`` for validation, and
trains :class:`TacotronWithSurprisal` (text + surprisal embedding
-> LSTM encoder -> attention + LSTMCell decoder -> linear head)
under MSE for ``num_epochs`` epochs. The checkpoint with the
lowest validation loss is saved to
``./best_model/best_model_<k_test>.pth`` and reloaded for the
test pass; per-test-utterance MSE is written to
``../podaci/mel results/results_surprisal_<k_test>.csv``
(consumed by ``results.py`` together with sister directories
``Results random`` and ``Results keras``) and a
training/validation loss plot is shown.
"""
import pandas as pd
import os
...
class CustomDataset(Dataset):
    """``torch.utils.data.Dataset`` view over the surprisal/mel table.

    Wraps the per-fold subset of the master DataFrame produced by
    ``calculate_mel_spectrum.py`` so that the random tokenized
    text, the GPT-2 surprisal values and the Mel target are
    served as PyTorch tensors.
    """

    def __init__(self, data):
        """Store the underlying DataFrame.

        Parameters
        ----------
        data : pandas.DataFrame
            Slice of the master table containing at least the
            tokenized text column, the surprisal values column and
            the Mel target column in the column order expected by
            :meth:`__getitem__`.
        """
        self.data = data

    def __len__(self):
        """Return the number of rows in the wrapped DataFrame."""
        return len(self.data)

    def __getitem__(self, idx):
        """Return the ``(text, surprisal, mel, idx)`` tensors for row ``idx``.

        The original DataFrame ordering
        ``(Audio Files, Text random, Text keras, Surprisal Values,
        Speaker, Emotion, Target Sentence, Fold, Mel Spectrum)`` is
        unpacked positionally; only the random text tokens, the
        surprisal vector and the Mel target are converted to
        :class:`torch.Tensor`. ``idx`` is returned alongside so
        per-batch metadata (audio path / speaker / emotion) can be
        joined back from the parent DataFrame.
        ...
        -------
        tuple
            ``(text_tensor, surprisal_tensor, mel_target_tensor, idx)``.
        """
...
class TacotronWithSurprisal(nn.Module):
    """Tacotron-style spectrogram predictor conditioned on surprisal.

    Architecture: token embedding plus a linear projection of the
    per-token surprisal value, summed and fed into a single-layer
    LSTM encoder. A Bahdanau-style additive attention mechanism
    drives an :class:`torch.nn.LSTMCell` decoder; the stacked
    decoder hidden states are flattened and pushed through a
    two-layer MLP whose output is reshaped to a Mel spectrogram
    of size ``(80, 250)``.
    """

    def __init__(self, vocab_size, embedding_dim, encoder_hidden_dim, decoder_hidden_dim, mel_dim, batch_size):
        """Build the embeddings, encoder, attention and decoder layers.

        Parameters
        ----------
        vocab_size : int
            Size of the token vocabulary used for the random
            tokenizer.
        embedding_dim : int
            Token / surprisal embedding dimension.
        encoder_hidden_dim : int
            Hidden size of the encoder LSTM.
        decoder_hidden_dim : int
            Hidden size of the decoder :class:`torch.nn.LSTMCell`.
        mel_dim : int
            Number of Mel bands; kept for API symmetry but the
            output shape is fixed to ``(80, 250)``.
        batch_size : int
            Batch size; stored as a constructor argument and used
            indirectly via :meth:`forward`'s dynamic ``text.size(0)``.
        """
    ...
    def forward(self, text, surprisal_values):
        """Predict a Mel spectrogram from text tokens and surprisals.

        Parameters
        ----------
        text : torch.Tensor
            Long tensor of token ids of shape ``(batch, seq_len)``.
        surprisal_values : torch.Tensor
            Float tensor of per-token surprisal values of shape
            ``(batch, seq_len)``.

        Returns
        -------
        torch.Tensor
            Predicted Mel spectrograms of shape ``(batch, 80, 250)``.
        """
        # Embed text
