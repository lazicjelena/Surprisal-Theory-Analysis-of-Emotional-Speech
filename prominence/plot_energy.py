...
Created on Mon Aug 19 09:23:01 2024

@author: Jelena

Pipeline role
-------------
Energy-modulation plotting script for the ``Prominence/`` analysis.
Reads ``../podaci/prominence_data.csv`` (built by
``prominence_build_dataset.py``), uses the shared helper
:func:`analysis_utils.extraxt_parameter_over_emotion` to align
each emotional utterance's ``prominence`` value with the same
speaker/sentence/word's neutral counterpart, and draws two
figures: a 2x4 grid of per-emotion / per-gender 2-D scatter plus
linear fit (neutral vs. emotional prominence) and a 2x4 grid of
3-D scatters with a fitted regression plane on
``(neutral prominence, GPT-2 surprisal) -> emotional prominence``.
Output is purely visual; no CSVs are written.
"""
...
fig.suptitle('Energy Moulations over Different Emotional States', fontsize=30)
...
for emotion in [1, 2, 3, 4]:
    ...
    # Plot the points
    ax.scatter(df['prominence'], df['surprisal GPT'], df[emotion], color='r', label=emotions[emotion])
    
    # Plot the plane
    xx, yy = np.meshgrid(np.linspace(X['prominence'].min(), X['prominence'].max(), 20),
    ...
    # Plot the points
    ax.scatter(df['prominence'], df['surprisal GPT'], df[emotion], color='b', label=emotions[emotion])
