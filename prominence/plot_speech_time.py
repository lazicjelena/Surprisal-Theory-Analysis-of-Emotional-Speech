...
Pipeline role
-------------
Spoken-word-duration modulation plotting script for the
``Prominence/`` analysis. Reads
``../podaci/prominence_data.csv`` (built by
``prominence_build_dataset.py``), aligns each emotional row with
its neutral counterpart via
:func:`analysis_utils.extraxt_parameter_over_emotion` on
``"duration"`` and renders two figures: a 2x4 per-emotion /
per-gender scatter plus linear fit (neutral vs. emotional
duration) and a 2x4 grid of 3-D scatters with a fitted
regression plane on
``(neutral duration, GPT-2 surprisal) -> emotional duration``.
Output is purely visual; no CSVs are written. Note: the file
name contains a Cyrillic ``e`` (U+0435) instead of Latin ``e``;
this is preserved verbatim under the project's zero-change rule.
"""
