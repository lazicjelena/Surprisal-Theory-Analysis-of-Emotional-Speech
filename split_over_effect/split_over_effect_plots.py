...
Pipeline role
-------------
Final split-over plotting script. Reads the per-surprisal
``\Delta\log\mathcal{L}`` summary tables produced by
``surprisal_results.py``
(``../podaci/split-over results/<Surprisal X>_results.csv``,
one row per ``(speaker gender, emotion, k)``) and renders the
publication figures: per-emotion / per-gender scatters of
``\Delta\log\mathcal{L}`` versus the surprisal lag order
``k`` in ``[0..4]``, in two color groups (autoregressive: GPT-2
vs. Yugo vs. ngram-3; bidirectional: BERT vs. BERTic), each in
both English and Serbian variants. Output is purely visual; no
CSVs are written.
"""
