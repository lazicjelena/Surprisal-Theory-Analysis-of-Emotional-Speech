# -*- coding: utf-8 -*-
"""bertic_surprisal_estimation.py

Per-word surprisal estimation using BERTic, the Bosnian/Croatian/
Montenegrin/Serbian masked language model from CLASSLA. Converted from
``BERTic_surprisal_estimation.ipynb`` (P-013-NB).

Pipeline role
-------------
Sister script of :mod:`bert_surprisal_estimation` for the same masked-LM
pipeline, but using the South-Slavic-tuned ``classla/bcms-bertic``
checkpoint instead of multilingual BERT. Iterates over every sentence
in ``../podaci/target_sentences.csv``, masks each token in turn, asks
the BERTic model for the probability of the original token at the
masked position, merges WordPiece sub-tokens back into orthographic
words and writes the per-word surprisal column ``Surprisal BERTic`` to
``../podaci/word_surprisals_bertic.csv``. The output is consumed by the
...
OUTPUT_CSV_PATH = os.path.join('..', 'podaci', 'word_surprisals_bertic.csv')
...
def mask_each_word(sentence):
    ...
    Tokenizes ``sentence`` with the module-level BERTic tokenizer
...
def estimate_masked_probability(sentence, candidate_word, model=model, tokenizer=tokenizer):
    ...
    model : transformers.BertForMaskedLM, optional
        BERTic MLM checkpoint. Defaults to the module-level
        ``model``.
...
def concatenate_words_and_probabilities(words, probabilities):
    """Merge BERTic WordPiece sub-tokens back into orthographic words.

    BERTic splits unknown tokens into ``"head"`` plus one or more
    ...
    ----------
    words : list of str
        WordPiece tokens as produced by the BERTic tokenizer.
...
def main():
    """Build and save the per-word BERTic surprisal table.
    ...
    ``(Sentence, Word, Surprisal BERTic)`` table to
    ...
    # Create a DataFrame
    df = pd.DataFrame({'Sentence': target_sentence_list, 'Word': words_list, 'Surprisal BERTic': probabilities_list})
